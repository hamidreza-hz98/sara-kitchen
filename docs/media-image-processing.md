...
The processor's general input cap is 10 MiB, 32 million pixels, and 8000 pixels per side; the **serverless inline route has stricter limits** in [`media-serverless-processing.md`](media-serverless-processing.md). Header metadata is inspected before decoding; actual decoding uses Sharp's pixel limit and `failOn: "warning"`. Variants run sequentially with bounded native timeouts. Processing errors return stable codes without raw decoder details. The original upload remains in the private bucket under the upload policy; only checked, processed variants should later be eligible for public delivery. This is not malware scanning, a sandbox, or a replacement for storage quotas. The worker/runtime should also have memory and CPU limits.
