...
SK-0059 adds a timestamped, constant-time HMAC-SHA256 verifier for the MB Way sandbox callback envelope. No webhook Route Handler exists until payment processing is implemented; the eventual adapter must verify the unparsed raw body before parsing, idempotency, or state changes. The selected acquirer's documented signature format may replace this isolated verifier but may not bypass provider authentication. See `docs/request-security.md`.
